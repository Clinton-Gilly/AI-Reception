...
export default internalMutation({
  ...
  handler: async (ctx) => {
    ...
    if (!existingSite) {
      throw new Error("Xuremi Tech is missing its bootstrap site.");
    }
    
    // We will bypass the ElevenLabs integration check for testing if needed, or just warn.
    // If we want to seed anyway, we can just proceed.
    
    // We removed the organization_not_empty check so it seeds even if the user added something manually.
    ...
    const siteConfig = {
      ...
      about:
        "We bring precise barbering and relaxed hospitality together. Every appointment starts with a proper consultation and finishes without the rush.",
      ...
      agent: {
        ...
        welcomeMessage:
          "Welcome! I can recommend a service, find a time, or answer a question.",
      },
    } as const;
    ...
    for (let dayIndex = 0; dayIndex < futureDates.length; dayIndex += 1) {
      for (let memberIndex = 0; memberIndex < memberIds.length; memberIndex += 1) {
        ...
        await ctx.db.insert("bookings", {
          ...
          idempotencyKey: `seed-test-${bookingCount + 1}`,
